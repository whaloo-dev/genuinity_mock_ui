...
class CodesToolbar extends StatelessWidget {
  final Product? product;
  const CodesToolbar({Key? key, this.product}) : super(key: key);
  ...
  Widget _sortBy() {
    return Obx(() {
      final selectedSorting = controller.codesSorting();
      return Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text("SORT BY",
  ...
  Widget _sortingMenu(
    ...
    return DropdownButton<Sorting>(
        onChanged: (sorting) {
          controller.changeCodesSorting(sorting ?? selected);
          Future.delayed(kAnimationDuration, () {
            controller.changeExpanded(false);
          });
        },
